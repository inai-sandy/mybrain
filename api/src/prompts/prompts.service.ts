...
- If they ask something you don't know, that needs Sandeep's own decision, or is outside these items: set "needsSandeep": true, and reply that you'll pass it to Sandeep and he'll get back to them. NEVER make up an answer. If it is clearly about ONE of the numbered items above, put that number in "needsItem" so Sandeep sees which piece of work it is about; if it is general or you cannot tell, leave "needsItem" null rather than guessing.
...
- DAILY REPORTS: an item tagged [daily report] is a standing update they owe EVERY working day — it is never "finished", so NEVER put it in "done". If their latest message contains today's actual report for such an item (real content: figures, names, hours, quantities, a genuine status or a clear "nothing to report today"), list those numbers in "statusToday". A bare "ok", "yes", "sure" or "will send" is NOT a report — leave it out and ask them for the actual update.
...
{"send": true or false, "reply": "<one message — only if send is true>", "needsSandeep": true or false, "needsItem": null or <number of the item it is about>, "done": [<numbers of items they say are finished, or empty>], "statusToday": [<numbers of [daily report] items whose actual update is in this message, or empty>], "promise": null or {"item": <number>, "date": "YYYY-MM-DD"}}`,
  },
  {
...
Reply with ONLY the summary line(s), nothing else.`,
  },
  {
    ...
    description: "Splits an EMO voice note into intents (task, brief, close, reminder, story, agent, search, research, meeting, idea, note) and classifies each. Your transcript is added automatically. ⚠️ Keep the JSON shape intact — use Reset if unsure.",
