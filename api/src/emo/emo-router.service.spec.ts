...
describe('EmoRouterService (BEA-863)', () => {
  it('splits one transcript into multiple cards across lanes', async () => {
    ...
    const out = await new EmoRouterService(prismaStub, llm, svc, searchStub, taskStub, ideaStub, reminderStub, meetingStub, researchStub).route('… long dump …', { source: 'browser' });
  ...
  it('marks story/note as done (the card is the result), actionable lanes as cooking', async () => {
    ...
    const { svc } = makeCards();
    await new EmoRouterService(prismaStub, llm, svc, searchStub, taskStub, ideaStub, reminderStub, meetingStub, researchStub).route('met the vendor, felt good');
    // (use svc's own created via a fresh pair)
    const pair = makeCards();
    await new EmoRouterService(prismaStub, llm, pair.svc, searchStub, taskStub, ideaStub, reminderStub, meetingStub, researchStub).route('met the vendor, felt good');
  ...
  it('files a fallback note card when the LLM output is unusable — nothing is lost (BEA-863)', async () => {
    ...
    const { svc, created } = makeCards();
    const out = await new EmoRouterService(prismaStub, llm, svc, searchStub, taskStub, ideaStub, reminderStub, meetingStub, researchStub).route('some rambling voice note');
  ...
  it('device search runs immediately — no clarify questions from the EMO device (BEA-938)', async () => {
    ...
    const { svc } = makeCards();
    await new EmoRouterService(prismaStub, llm, svc, searchStub, taskStub, ideaStub, reminderStub, meetingStub, researchStub).route('cctv market', { source: 'emo-device' });
  ...
  it('forced idea lane goes straight to the Ideas organiser (BEA-950)', async () => {
    ...
    const { svc } = makeCards();
    await new EmoRouterService(prismaStub, llm, svc, searchStub, taskStub, ideaStub, reminderStub, meetingStub, researchStub).route('an app that reminds plants to water themselves', { source: 'emo-device', lane: 'idea' });
