...
- Some actions have a SILENT-DEFAULT trap: left without a limit argument they answer their own tiny default and lose the rest without a word (Gmail's fetch emails answers exactly ONE message when max_results is left out — a real night was lost to it). When a card's notes or lessons say the bare default is tiny, every planned fetch through that action MUST set the limit argument (max_results / limit / page_size) explicitly — never leave it to the default — and when the owner asks for "all" or "everything", page beyond that limit too.
