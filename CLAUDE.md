...
- **Dictation cleanup and the EMO router are named helpers with OWNER-FACING setting rows (BEA-1624).** `'voice-cleanup'` → `voice.cleanup.model`, `'emo-router'` → `emo.router.model` — `LlmService.HELPER_SETTING_KEYS` is the map, read by `helperModel()` AND `setHelperModel()`, so the Voice screen (`PUT /api/voice/cleanup-model`), the EMO screen (`PUT /api/emo/settings {routerModel}`) and `PUT /api/llm-config/helper/<key>` all move one row. Default `openai/gpt-5.6-terra` (the owner's pick), a blank/junk row falls back to that default and never to a cheaper model; both are INTERACTIVE (one call, no retry). The curated picker list lives ONCE in `llm/curated-models.ts` (`EMO_MODELS` re-exports it). Never write a model id inline in `voice.service.ts` / `emo-router.service.ts` — `owner-model-choice.spec.ts` fails on it.
...
- **TWO EMO device tokens are live at once, on purpose (`specs/briefs/DEVICE-TOKEN-ROTATION.md`).** The shared device key was exposed in a public firmware repo, and the owner's prototypes (pendant, capsule, watch, K10) can only be reflashed one at a time — so `regenerateDeviceToken()` no longer overwrites: it moves the current key to `emo.device.token.prev` with `emo.device.token.prev.until` = now + 30 days and mints a new `emo.device.token`. `verifyDeviceToken(presented, route?)` compares against BOTH candidates every time (`constantTimeEquals`, no early return that leaks which matched) and refuses `prev` past its deadline. A request that authenticates on the OLD key writes `prev.lastseen`/`prev.count` (fire-and-forget, awaited in specs through `prevUseWrite`) and logs ONE info line naming the route — so the owner can find the board still on old firmware. **No token value is ever logged or returned for `prev`**: `deviceTokenView()` answers `{token, previous:{active,until,lastSeenAt,count}}`, never the old value. Settings → EMO → Device token has **Rotate the key** and, while the old key lives, "the old key was last used … (N requests)" + **Revoke now** → `POST /api/auth/device-token/revoke-previous`, which is the LAST step of a rotation. **A device key manages nothing, least of all itself**: all three device-key routes are `@SessionOnly()` (`api/src/auth/session-only.decorator.ts`, enforced in `AuthGuard` before the `X-Device-Token` branch) — otherwise the leaked key would read its own replacement out of `GET /auth/device-token` during the grace period and the rotation would buy nothing (`device-key-routes.spec.ts` locks it). Rotating twice while a previous key is still live is REFUSED with a plain reason unless `{force:true}` — a second rotation would push the first old key out of the window on the spot, which is the lockout this exists to prevent. Traps: the guard passes `req.path` only (a query string can carry secrets); the tokens are cached in memory for a synchronous guard, so anything that changes them must update memory AND the Setting rows together; the sighting write is queued on `prevUseWrite` so two devices in one tick cannot walk `lastseen` backwards.
