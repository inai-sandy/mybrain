...
import { MessageCircle, Plus, Send, X, ArrowLeft, ExternalLink, Sparkles, Trash2, Globe, Bookmark, Lightbulb, Activity as ActivityIcon, FileText, Wand2 } from 'lucide-react';
  ...
  const listPane = (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto space-y-1">
        {sessions.length === 0 && <p className="text-sm text-zinc-400 p-4 text-center">No chats yet — tap “New” to talk to your brain.</p>}
        {sessions.map((s) => {
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-4">
        {active.messages.length === 0 && (
          <div className="text-center text-sm text-zinc-400 mt-10">
            ...
            Ask anything about your {sc?.label.toLowerCase()}.
          </div>
        )}
        {active.messages.map((m) => (
          <div key={m.id} className={m.role === 'user' ? 'flex justify-end' : 'flex justify-start'}>
            <div className={'max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm ' + (m.role === 'user' ? 'bg-emerald-600 text-white' : 'bg-zinc-100 dark:bg-zinc-800')}>
