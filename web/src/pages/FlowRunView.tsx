...
export function FlowRunView() {
  ...
  const steps = items.filter((i) => i.kind !== 'output' && i.kind !== 'note' && i.kind !== 'question');
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      <header className="flex items-center justify-between gap-3">
        <h1 className="text-lg font-bold">Flow run</h1>
        <span className={'rounded-full px-2 py-0.5 text-xs font-medium ' + (run.status === 'done' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : run.status === 'failed' ? 'bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400' : 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400')}>{run.status === 'running' ? 'running…' : run.status}</span>
      </header>
      ...
      {(run.terminal?.length > 0 || run.status === 'running') && (
        <details open={run.status === 'running'} className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950">
          ...
          <div className="max-h-72 overflow-auto border-t border-zinc-800 px-3 py-2 font-mono text-xs leading-relaxed text-zinc-300">
            ...
            {(run.terminal || []).length === 0 && run.status === 'running' && <div className="text-zinc-500">starting…</div>}
          </div>
        </details>
      )}

      <div className="space-y-2">
        {steps.length === 0 && run.status === 'running' && <div className="flex items-center gap-2 text-sm text-zinc-500"><Loader2 className="h-4 w-4 animate-spin" />Starting…</div>}
        {steps.map((i) => (
          <details key={i.nid} className="rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900" open={i.status === 'running'}>
            <summary className="flex cursor-pointer list-none items-center gap-2 px-3 py-2 text-sm [&::-webkit-details-marker]:hidden">
              <NodeStatus s={i.status} />
              <span className="min-w-0 flex-1 truncate font-medium">{i.label || i.kind}</span>
              <span className="shrink-0 text-xs text-zinc-400">{i.kind}{i.status === 'skipped' ? ' · skipped' : ''}</span>
            </summary>
      ...
      {run.status === 'done' && run.finalOutput && (
        <div className="rounded-2xl border border-emerald-300 bg-emerald-50 p-4 dark:border-emerald-500/30 dark:bg-emerald-500/10">
          <div className="mb-2 flex items-center justify-between gap-2">
            <span className="text-xs font-medium uppercase tracking-wide text-emerald-700 dark:text-emerald-400">Final output</span>
