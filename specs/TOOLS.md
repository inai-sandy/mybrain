...
**"No action skipped" is now the standard for every provider.** BEA-1354 lifted the 60-per-service cap BEA-1345 put on Composio (GitHub showed 36 of 871): `listActions()` walks the whole cursor-paged list (one 1,000-a-page request per service, cached 5 min), the catalog serves its last good copy at once and re-reads BEHIND the answer when it is old (`ToolCatalogService.serviceTools`, generation-aware so a connect/disconnect is seen at once, boot-warmed), and the 8s budget + last-good fallback stay. Pickers fold per service with search-within-service and 40-a-page (`web/src/ui/ServiceFolds.tsx`); every prompt that shows service actions gets a keyword-ranked slice, never the lot (`api/src/tools/tool-shortlist.ts` — Chat's second pick, the flow planner, the job builder, EMO's tool pick). Gates are computed over the full set.
